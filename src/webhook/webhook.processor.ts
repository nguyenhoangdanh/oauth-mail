...
export class WebhookProcessor extends WorkerHost {
  ...
  // Instead of @Process use process method
  async process(job: Job<any>): Promise<void> {
    this.logger.debug(
      `Processing webhook delivery job ${job.id} (attempt ${job.attemptsMade + 1})`,
    );

    const { webhookId, deliveryId, payload } = job.data;

    try {
      await this.webhookService.deliverWebhook(webhookId, deliveryId, payload);
      this.logger.debug(
        `Webhook delivery job ${job.id} completed successfully`,
      );
    } catch (error) {
      this.logger.error(
        `Error processing webhook delivery job ${job.id}: ${error.message}`,
        error.stack,
      );
